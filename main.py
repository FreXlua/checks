...
def init_counter_db():
    if not os.path.exists('counter.db'):
        ...
        cur = con.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS counter (count INTEGER DEFAULT 0)")
...
def start_message(message):
    ...
    if sql.fetchone() is None:
        ...
        cur.execute("UPDATE counter SET count = ?", (new_count,))
        db.commit()
        con.commit()
        bot.send_message(
            message.chat.id,
            '🧾Привет! У меня ты сможешь сделать фейк чеки популярных приложений. Надеюсь тебе понравится :)\n⚠️Воспользуйся кнопками для управления ботом\nПо всем вопросам: @frexTC',
...
def main(message):
    if message.text == '📝Отрисовка (Чеки, балансы)':
        ...
        bot.send_message(message.chat.id, "Заказать рекламу: @frexTC", reply_markup=main_keyboard())
        bot.register_next_step_handler(message, main)
